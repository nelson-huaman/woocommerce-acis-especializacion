<?php
   ...
   foreach ($product->get_available_variations() as $variation):
      ...
      foreach (wc_get_product($variation['variation_id'])->get_variation_attributes() as $nombreVariacion):
         if($descuento === 0) {
            $option = "{$nombreVariacion}: Cuota 1 de {$variation['variation_cuotas_field']}";
         } else {
            $option = "{$nombreVariacion}: Cuota 1 de {$variation['variation_cuotas_field']} (Inc. {$descuento}% dscto.)";
         }
